...
$resulta=mysql_query('select * from goods where status=0 and state=0');
...
$sql1="select *,(($x*typemodule1+$y*typemodule2)/(sqrt($x*$x+$y*$y)*sqrt(typemodule1*typemodule1+typemodule2*typemodule2)))as cos from goods where status=0 and state=0 order by cos desc limit $offset,$Page_size";
$sql2="select *,(round(6378.138*2*asin(sqrt(pow(sin((lat1*pi()/180-lat2*pi()/180)/2),2)+cos(lat1*pi()/180)*cos(lat2*pi()/180)*
pow(sin((lon1*pi()/180-lon2*pi()/180)/2),2)))*1000)+round(6378.138*2*asin(sqrt(pow(sin((lat1*pi()/180-'$lat'*pi()/180)/2),2)+cos(lat1*pi()/180)*cos(lat2*pi()/180)*
pow(sin((lon1*pi()/180-'$lon'*pi()/180)/2),2)))*1000)) as distance from goods where status=0 and state=0 order by distance asc limit $offset,$Page_size";
switch ($_GET[prefer]) {
	case "习惯优先":
		$sql=$sql1;
		break;
